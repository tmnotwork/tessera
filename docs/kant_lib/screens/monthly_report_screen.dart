...
class MonthlyReportScreen extends StatefulWidget {
  ...
  const MonthlyReportScreen({super.key, this.initialDate});
...
class _MonthlyReportScreenState extends State<MonthlyReportScreen> {
  ...
  late DateTime _monthStart; // YYYY-MM-01
  bool _loading = true;
  final List<int> _plannedByWeek = [];
  final List<int> _actualByWeek = [];
  ...
  List<String> _projectIdsSorted() {
    final ids =
        <String>{..._plannedByProject.keys, ..._actualByProject.keys}.toList();
    int totalOf(String pid) =>
        (_plannedByProject[pid] ?? 0) + (_actualByProject[pid] ?? 0);
    ids.sort((a, b) {
  ...
  void initState() {
    ...
    _monthStart = DateTime(base.year, base.month, 1);
    _groupingMode = AppSettingsService.getString(AppSettingsService.keyReportMonthlyGrouping) ?? 'time';
  ...
  @override
  void didUpdateWidget(covariant MonthlyReportScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    final base = widget.initialDate ?? DateTime.now();
    final nextMonthStart = DateTime(base.year, base.month, 1);
    if (nextMonthStart.year != _monthStart.year || nextMonthStart.month != _monthStart.month) {
  ...
  Future<void> _load({bool runSync = true}) async {
    ...
    // 同期完了後、ローカル集計を表示
    final data = _reportRepo.loadMonthlySummary(_monthStart);
    if (mounted) {
      setState(() {
        _plannedByWeek
          ..clear()
          ..addAll(data.weekBuckets.map((bucket) => bucket.plannedMinutes));
        _actualByWeek
          ..clear()
          ..addAll(data.weekBuckets.map((bucket) => bucket.actualMinutes));
        _plannedByProject
          ..clear()
  ...
  void _changeMonth(int deltaMonths) {
    setState(() {
      _monthStart = DateTime(_monthStart.year, _monthStart.month + deltaMonths, 1);
    });
    unawaited(_load());
  }

  String _periodLabel() {
    return DateFormat('yyyy/MM').format(_monthStart);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final weeks = List.generate(_plannedByWeek.length, (i) => i);
      ...
      if (_groupingMode == 'time') {
        final maxVal = [
          ..._plannedByWeek.map((e) => e.toDouble()),
          ..._actualByWeek.map((e) => e.toDouble())
        ].fold<double>(1.0, (p, v) => v > p ? v : p);
        dataMaxHours = (maxVal / 60.0);
        bars = List<BarChartGroupData>.generate(weeks.length, (i) {
          final planned = _plannedByWeek[i] / 60.0;
          final actual = _actualByWeek[i] / 60.0;
          final scheme = Theme.of(context).colorScheme;
        ...
        for (int i = 0; i < projectChartEntries.length; i++) {
          ...
          bars.add(BarChartGroupData(x: i, barsSpace: 6, barRods: [
            BarChartRodData(
                toY: ph,
                width: 12,
                borderRadius: BorderRadius.circular(2),
      ...
      // Bottom title is 2 lines (週範囲 or プロジェクト名) なので高さを確保する
      ...
      return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                            ...
                            child: DropdownButtonFormField<String>(
                              ...
                              onChanged: (value) async {
                                ...
                                await AppSettingsService.setString(
                                  AppSettingsService.keyReportMonthlyGrouping,
                                  value,
                                );
                              },
                            ),
            ...
            _buildMonthlyTotalsList(context),
          ],
        ),
      );
    });
  ...
  Widget _buildMonthlyTotalsList(BuildContext context) {
    final ids = _projectIdsSorted();
    if (ids.isEmpty) {
        ...
        Container(
          ...
          child: Row(
            children: [
              Expanded(
                flex: 5,
                child: Text('プロジェクト', style: Theme.of(context).textTheme.labelMedium),
              ),
              Expanded(
                flex: 3,
                child: Align(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                ...
                child: Align(
                  alignment: Alignment.centerRight,
                  child: Text(_formatMinutes(totalPlanned), style: const TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
              Expanded(
                flex: 3,
                child: Align(
                  alignment: Alignment.centerRight,
                  child: Text(_formatMinutes(totalActual), style: const TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
              Expanded(
                flex: 3,
                child: Align(
                  alignment: Alignment.centerRight,
                  child: Text(_formatMinutes(totalDiff), style: const TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
              Expanded(
                flex: 3,
