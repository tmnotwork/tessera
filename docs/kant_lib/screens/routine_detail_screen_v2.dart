...
class RoutineDetailScreenV2 extends StatefulWidget {
  final RoutineTemplateV2 routine;
  final bool embedded;

  const RoutineDetailScreenV2(
      {super.key, required this.routine, this.embedded = false});

  @override
  State<RoutineDetailScreenV2> createState() => _RoutineDetailScreenV2State();
}
...
class _RoutineDetailScreenV2State extends State<RoutineDetailScreenV2> {
  ...
  final RoutineMutationFacade _mutationFacade =
  ...
  Future<void> _repairOrphanRoutineData() async {
    ...
    final orphanBlocks = RoutineBlockV2Service.getAllByTemplate(widget.routine.id)
  ...
  Future<void> _bootstrapTemplateIfNeeded({bool showSpinner = true}) async {
    ...
    try {
      final result = await _editorRepository.bootstrapTemplateV2(widget.routine);
      final hasBlocks =
  ...
  @override
  void didUpdateWidget(covariant RoutineDetailScreenV2 oldWidget) {
  ...
  Widget build(BuildContext context) {
    final content = Stack(children: [
      StreamBuilder<RoutineEditorSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!_hasInitializedExpansionState && blocks.isNotEmpty) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor:
            _parseHexColor(widget.routine.color) ?? Theme.of(context).colorScheme.primary,
        ...
        actions: [
          ...
          StreamBuilder<RoutineEditorSnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final expandableIds = _collectExpandableBlockIds(
                data,
              ...
              final allExpanded = _areAllBlocksExpanded(
                data,
                useMobileCards: useMobileCards,
              );
              return IconButton(
                tooltip: allExpanded ? '全ブロックを折りたたむ' : '全ブロックを展開',
                icon: Icon(
                  allExpanded ? Icons.unfold_less : Icons.unfold_more,
                ),
                onPressed: () => _toggleAllBlocksExpansion(
                  data,
                  useMobileCards: useMobileCards,
                ),
              );
    ...
    return Padding(
      ...
      child: Center(
        child: FilledButton.icon(
          onPressed: _isBootstrapping ? null : _showAddBlockDialog,
  ...
  bool _shouldUseMobileTaskCards(BuildContext context) {
    // 表は横に列が多く、実表示幅が狭いと破綻するのでカード縦積みに切り替える（幅のみ判定）。
  ...
  Widget _buildMobileScheduledTaskCard(
    ...
    final title =
        (task.name.trim().isNotEmpty) ? task.name.trim() : '(無題)';
    final startText =
    ...
    final durationText =
        (scheduledTask.durationMinutes > 0) ? '${scheduledTask.durationMinutes}分' : '';
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    lineWithIcon(Icons.schedule, '$rangeText${durationText.isEmpty ? '' : ' / $durationText'}'),
                    if (subtitle.isNotEmpty) lineWithIcon(Icons.folder, subtitle),
  ...
  Widget _buildTaskTableHeader(BuildContext context) {
    ...
    Widget headerCell({
      ...
      final decorated = Container(
        ...
        padding: padding,
        decoration: BoxDecoration(
          color: theme.colorScheme.surfaceContainerHighest.withOpacity(theme.brightness == Brightness.light ? 1 : 0.2),
          border: Border(
  ...
  Widget _buildTaskCard(
    ...
    final bool blockCrossesMidnight =
        _timeOfDayToMinutes(block.endTime) <= _timeOfDayToMinutes(block.startTime);
  ...
  TextEditingController _ensureSubProjectControllerV2(
    ...
    if (_pendingSubProjectClearByTaskId.contains(task.id)) {
      if ((task.subProjectId ?? '').isEmpty && (task.subProject ?? '').isEmpty) {
  ...
  TextEditingController _ensureRepresentativeSubProjectController(
    ...
    if (_pendingSubProjectClearByBlockId.contains(block.id)) {
      if ((block.subProjectId ?? '').isEmpty && (block.subProject ?? '').isEmpty) {
  ...
  void _purgeUnusedControllers(Set<String> activeTaskIds) {
    ...
    _pendingSubProjectClearByTaskId.removeWhere((id) => !activeTaskIds.contains(id));
    _pendingProjectClearByTaskId.removeWhere((id) => !activeTaskIds.contains(id));
    _pendingLocationCommitByTaskId.removeWhere((id, _) => !activeTaskIds.contains(id));
    _pendingNameCommitByTaskId.removeWhere((id, _) => !activeTaskIds.contains(id));
  }
  ...
  void _purgeUnusedBlockControllers(Set<String> activeBlockIds) {
    ...
    _pendingSubProjectClearByBlockId.removeWhere((id) => !activeBlockIds.contains(id));
    _pendingProjectClearByBlockId.removeWhere((id) => !activeBlockIds.contains(id));
    _pendingLocationCommitByBlockId.removeWhere((id, _) => !activeBlockIds.contains(id));
  }
  ...
  String _formatTimeRange(TimeOfDay start, TimeOfDay end) {
    ...
    final crossesMidnight = _timeOfDayToMinutes(end) <= _timeOfDayToMinutes(start);
                    ...
                    children: [
                      Expanded(
                        child: TextField(
                          ...
                          decoration: buildDecoration('\u4f11\u6182\u6642\u9593( \u5206 )',
                              hintText: '0'),
                          keyboardType: TextInputType.number,
                          onChanged: (_) {
                            if (updatingWorkBreak) return;
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: buildDecoration('\u7a3c\u50cd\u6642\u9593( \u5206 )',
                              hintText: '0'),
                          keyboardType: TextInputType.number,
                          onChanged: (_) {
                            if (updatingWorkBreak) return;
  ...
  Future<void> _editBlock(rbv2.RoutineBlockV2 b) async {
    ...
    final breakCtrl =
        TextEditingController(text: normalizedBreak.toString());
  ...
  Future<void> _addTaskToBlock(rbv2.RoutineBlockV2 b) async {
    // Inbox と同じく、まず行（タスク）を追加してから編集する。
    final now = DateTime.now();
    final existingTasks = RoutineDatabaseService.getTasksForBlock(b.id);
    int nextOrder = 0;
    for (final task in existingTasks) {
      if (task.order >= nextOrder) nextOrder = task.order + 1;
    }

    final t = rtv2.RoutineTaskV2(
      id: now.millisecondsSinceEpoch.toString(),
      routineTemplateId: b.routineTemplateId,
      routineBlockId: b.id,
      name: '', // まず空で追加（セル側のplaceholderで表示）
    ...
    await _mutationFacade.addTask(t);
    if (mounted) setState(() {});
  }
