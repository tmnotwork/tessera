...
import '../widgets/inbox/inbox_task_table_test.dart';
...
class InboxTestScreenController implements InboxControllerInterface {
  InboxTestScreenController();

  _InboxTestScreenState? _state;
  @override
  ...
  Future<void> requestSync() async {
    ...
    await state._syncInboxEnsuringAuth();
  }

  void _attach(_InboxTestScreenState state) {
    if (_disposed) return;
    _state = state;
    _setSyncing(state._isSyncing);
  }

  void _detach(_InboxTestScreenState state) {
    if (_disposed) return;
    if (_state == state) {
      _state = null;
      _setSyncing(false);
...
class InboxTestScreen extends StatefulWidget {
  final InboxTestScreenController? controller;

  const InboxTestScreen({super.key, this.controller});
...
class _InboxTestScreenState extends State<InboxTestScreen> {
  ...
  StreamSubscription<List<DayVersionDoc>>? _inboxVersionWatchSub;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // tasks全件snapshotsを避け、dayVersionsを軽量トリガーとして監視する
      try {
        _inboxVersionWatchSub?.cancel();
        _inboxVersionWatchSub =
            TimelineVersionService.watchRecentChanges(limit: 30).listen((_) {
          _versionDebounce?.cancel();
          _versionDebounce = Timer(const Duration(milliseconds: 600), () async {
            if (!mounted) return;
            try {
              ...
              await SyncManager.syncIfStale(
                ...
                minFreshDuration: const Duration(seconds: 10),
              );
  ...
  @override
  void didUpdateWidget(covariant InboxTestScreen oldWidget) {
  ...
  Future<void> _syncInboxEnsuringAuth({int maxAttempts = 3}) async {
    ...
    try {
      for (int attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          final result = await SyncContext.runWithOrigin(
            'InboxTestScreen.manualSync',
                  ...
                  children: [
                    const Expanded(
                      child: SingleChildScrollView(
                        ...
                        child: InboxTaskTableTestWidget(),
                      ),
                    ),
                    if (runningTask != null)
                      RunningTaskBar(
                  ...
                  if (runningTask != null)
                    InkWell(
                      onTap: () {
                        showUnifiedScreenDialog<void>(
                          context: context,
                          builder: (_) => MobileTaskEditScreen(task: runningTask),
                        );
                      },
                      child: MobileRunningTaskBar(
                        runningTask: runningTask,
