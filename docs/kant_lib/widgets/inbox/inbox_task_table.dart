...
class InboxTaskTableWidget extends StatefulWidget {
  const InboxTaskTableWidget({
    super.key,
    this.filterRowTrailingActions,
    this.showAssigned = false,
  });

  /// 「割り当て済みも表示」の行の右端に並べるウィジェット（同期・CSVインポート・設定など）
  final List<Widget>? filterRowTrailingActions;

  /// 割り当て済みも表示するかどうか（フィルタ）。外側（AppBarなど）から渡す。
  final bool showAssigned;

  @override
  State<InboxTaskTableWidget> createState() => _InboxTaskTableWidgetState();
}

class _InboxTaskTableWidgetState extends State<InboxTaskTableWidget> {
  // 可変列の最小幅（PC）
  ...
  // レスポンシブ表示: タイムライン同様、幅に応じて列をフェードアウト＋縮退
  // 退避順: モード → サブプロジェクト
  static const double _modeColumnHideWidth = 1260; // モード列を0にする開始幅
  static const double _modeColumnShowWidth = 1340; // モード列を完全表示する幅
  static const double _subProjectColumnHideWidth = 1120; // サブプロジェクト列を0にする開始幅
  static const double _subProjectColumnShowWidth = 1200; // サブプロジェクト列を完全表示する幅
  ...
  // 実行時刻列（PC）: 文字が欠けない範囲で横幅と左右余白を詰める
  static const double _timeColumnMinWidth = 72;
  static const double _timeColumnHorizontalPadding = 2;
  static const String _timeSampleText = '88:88';
  // 既存行のコントローラ
  ...
  Color _resolveTableBackgroundColor(BuildContext context) {
    ...
    if (theme.brightness == Brightness.light) {
      // ライト系は「背景 + ごく薄い中立色」で固定し、テーマ固有の色相を拾わない。
  ...
  double _timeColumnWidth(BuildContext context) {
    final style = _cellTextStyle(context);
    // 「88:88」を基準に、カーソル等の余裕分も足して欠けない幅にする
    final textWidth = _measureTextWidth(
      context: context,
      text: _timeSampleText,
      style: style,
    );
    final width =
        textWidth + (_timeColumnHorizontalPadding * 2) + 18; // 余裕
    final rounded = width.ceilToDouble();
    return rounded < _timeColumnMinWidth ? _timeColumnMinWidth : rounded;
  }
  ...
  Future<void> _saveAndUnfocus(
    ...
    // Provider側のnotifyに依存せず、入力確定直後に表示を再評価して行の表示/非表示を即時反映する。
    // （例: 実行時刻を入力したらインボックスから消える）
    if (mounted) {
      setState(() {});
    }
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<TaskProvider>(
      builder: (context, taskProvider, child) {
        ...
        final baseTasks = all
            .where(
              (t) => taskProvider.shouldShowInboxTask(
                t,
                includeAssigned: widget.showAssigned,
              ),
            )
            .toList();
        final tasks = _orderTasksForDisplay(baseTasks);
        final timeColumnWidth = _timeColumnWidth(context);
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            // 表全体を「背景と少しだけ違う色」に統一（ライト系の色ブレ抑制）。
            final tableBackgroundColor = _resolveTableBackgroundColor(context);

            return Column(
              children: [
            // フィルタトグル行は新UI/旧UIのAppBar側に移動したため、ここでは表示しない
            // ヘッダー
            Container(
              height: 36,
              ...
              child: Column(
                children: [
                  ...List.generate(tasks.length, (index) {
                    ...
                    final rowSurface = tableBackgroundColor;
                    return Container(
                      height: 36,
                      decoration: BoxDecoration(
                        color: rowSurface,
                            ...
                            child: Container(
                              ...
                              child: SizedBox(
                                ...
                                child: TextField(
                                  ...
                                  onTap: () => _updateExecutionDate(context, t),
                                ),
                              ),
                            ),
                          ),
