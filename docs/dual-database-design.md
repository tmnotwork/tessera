...
【差分Pull（2回目以降）】
1. Pull開始直前に候補時刻を記録する:
     pull_start_at = DateTime.now().toUtc()  ← ここでキャプチャ
   ※ Pull完了後に now() を取ると、Pull中に更新されたレコードが
     次回 "updated_at > pull_end_at" で取れなくなる

2. Supabaseから以下の条件でレコードを取得:
     updated_at >= last_pull_at  ← >= (以上) を使うこと
   ※ > (より大きい) にすると、ちょうど last_pull_at と同じ
     タイムスタンプのレコードが永久に取得されなくなる
...
4. 削除検知:
   Supabaseで deleted_at が設定されたレコードをローカルに deleted=1 として反映。
   ただし deleted_at 設定時に updated_at も必ず更新されること（トリガー必須）。
   updated_at が更新されないと、差分Pull条件 "updated_at >= last_pull_at" で
   削除レコードが取得されず、ローカルのデータが残り続ける。

5. ページネーション（必須）:
   Supabase のデフォルト取得上限は 1000件。
   全件Pull・差分Pullともにページネーションを実装すること:
...
6. 全テーブルのPull完了後のみ → last_pull_at = pull_start_at を保存
   ※ pull_start_at（Pull開始前にキャプチャした時刻）を使う
   ※ 途中でエラーが発生した場合は last_pull_at を更新しない（次回に再Pull）
```

> **⚠ Supabase側に `deleted_at` カラムが必要**
> 現在のスキーマにはないため、全テーブルに追加するか、`deleted_records` テーブルを作成する。
> **追加要件**: `deleted_at` を設定するトリガーは同時に `updated_at` も更新すること。
> そうしないと差分Pullで削除が検知されない。
...
| 操作 | 現在 | 実装後（Web） | 実装後（Mobile/Desktop） |
|---|---|---|---|
| 一覧表示 | Supabase直接 | Supabase直接（変わらず） | ローカルDB（オフラインでも表示） |
| カード作成 | Supabase INSERT後に一覧リロード | Supabase INSERT後に一覧リロード | ローカルINSERT後に即反映、バックグラウンドでPush |
| カード編集 | Supabase UPDATE後にリロード | Supabase UPDATE後にリロード | ローカルUPDATE後に即反映 |
| カード削除 | Supabase DELETE後にリロード | Supabase DELETE後にリロード | ローカルにdeleted=1、UI即反映、バックグラウンドでPush |
| 並び替え | Supabase batch UPDATE後にリロード | Supabase batch UPDATE後にリロード | ローカル更新後に即反映 |

> **⚠ Mobile/Desktop はページリロード時の挙動が変わる**
> 現在: ページリロード → Supabaseから再取得 → 常に最新データ
> 実装後: ページリロード → ローカルDBから再取得 → **同期前は古いデータが見える可能性がある**
>
> これはオフラインファーストの意図的なトレードオフ。
> 最新化のタイミングは「同期完了時（SyncNotifier経由）」に変わる。
> Webは変わらず Supabase直接のため、ページリロードで常に最新が取れる。
